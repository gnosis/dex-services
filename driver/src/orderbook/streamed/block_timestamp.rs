...
use futures::{compat::Compat01As03, future::BoxFuture, FutureExt as _};
...
impl BlockTimestamp for Web3 {
    fn block_timestamp(&self, block_hash: H256) -> BoxFuture<Result<u64>> {
        let r = Compat01As03::new(self.eth().block(BlockId::Hash(block_hash)));
        let r = r.map(move |block| -> Result<u64> {
            let block = block

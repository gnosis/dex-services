...
use crate::graph::shortest_paths::new;
...
impl Orderbook {
    ...
    pub fn is_overlapping(&self) -> bool {
        ...
        Subgraphs::new(self.projection.node_indices().skip(1))
            .for_each_until(
                |token| match new(&self.projection, token, None) {
    ...
    pub fn reduce_overlapping_orders(mut self) -> ReducedOrderbook {
        Subgraphs::new(self.projection.node_indices()).for_each(|token| loop {
            let cycle = match new(&self.projection, token, None) {
    ...
    pub fn fill_market_ring_trade(&mut self, market: Market) -> Option<Ring> {
        ...
        loop {
            let cycle = match new(&self.projection, quote, None) {
                Ok(_) => break,
                Err(cycle) => cycle,
            };
    ...
    fn find_path_and_flow(
        ...
        let shortest_path_graph = new(&self.projection, start, None)?;

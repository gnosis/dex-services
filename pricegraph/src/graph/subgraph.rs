    ...
    G::EdgeWeight: FloatMeasure,
{
    ...
    /// connected to it.
    pub fn for_each(self, graph: G, mut f: impl FnMut(G::NodeId) -> ShortestPathGraph<G>) {
        self.for_each_until(graph, |node| <ControlFlow<G, ()>>::Continue(f(node)));
    }
    ...
    pub fn for_each_until<T>(
        self,
        graph: G,
        mut f: impl FnMut(G::NodeId) -> ControlFlow<G, T>,
    ) -> Option<T> {

...
class StateTransition(NamedTuple):
    ...
    @classmethod
    def fromDictionary(cls, data: Dict[str, Any]) -> StateTransition:
...
class Deposit(NamedTuple):
    ...
    @classmethod
    def fromDictionary(cls, data: Dict[str, Any]) -> Deposit:
        assert data.keys() == {'accountId', 'tokenId', 'amount', 'slot', 'slotIndex'}, "Unexpected Event Keys"
        assert all(isinstance(val, int) for val in data.values()), "One or more of event values not integer"
        return Deposit(
            data['accountId'], 
            data['tokenId'], 
            data['amount'], 
...
class Withdraw(NamedTuple):
    ...
    @classmethod
    def fromDictionary(cls, data: Dict[str, Any]) -> Withdraw:
        assert data.keys() == {'accountId', 'tokenId', 'amount', 'slot', 'slotIndex'}, "Unexpected Event Keys"
        assert all(isinstance(val, int) for val in data.values()), "One or more of event values not integer"
        return Withdraw(
            data['accountId'], 
